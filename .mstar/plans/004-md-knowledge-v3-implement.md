...
- [ ] **Step 1:** Confirm 003 Research lock GO — `status: GO`, empty `blockers`, R1–R7 pass. If NO-GO, stop and escalate (do not implement).
- [ ] **Step 2:** Checkout `iteration/md-knowledge-v3` from `main`; record branch tip SHA.
- [ ] **Step 3:** Re-run byte baseline commands from primary spec; compare to 003 `byte_baseline`; record **pre-edit** totals in SDD path-contract.
- [ ] **Step 4:** Publish path contract: enumerate every path from touch list with action (`modify`/`create`/`consolidate-*`); explicit **forbidden** paths (non-list `pack/**`, `pack/generated/**` hand-edits, non-md).
- [ ] **Step 5:** Build **ledger alignment table**: each touch-list file → ledger rows + matrix adopt IDs; flag overturn rows for side-by-side evidence in Task 4.
...
- [ ] **Step 0:** For files marked overturn in ledger: capture **before** excerpt from `main` tip (or 08-11 doc) into SDD before editing.
- [ ] **Step 1:** Implement adopted concepts with checks/scenarios/evidence per brief §4 and §10.1–§10.6.
- [ ] **Step 2:** Consolidate ≥3 duplicated guidance sites into canonical refs (record source → target in SDD).
- [ ] **Step 3:** Self-review falsifiability: each new check has stated failing case; delete decoration.
- [ ] **Step 4:** Verify no touch-list path edited outside contract; no orphan new files.
...
- [ ] **Step 1:** §10.7–§10.12 adaptations with falsifiable criteria per per-file SDD contract.
- [ ] **Step 2:** `NOTICE.md` attributions for CC BY / Apache adaptations (paths + source SHA).
- [ ] **Step 3:** Routing tie-breakers: `rite-quick` vs `rite-build` (and compass AC-9 pairs) resolve to exactly one skill in `intent-map` or routing index.
- [ ] **Step 4:** Reviewer independence: each `devrites-*-reviewer` touched states what it must not assume from implementer.
- [ ] **Step 5:** Prove/seal fail-closed: missing material evidence blocks seal per strengthened prove guidance (smoke scenario documented in SDD).
...
- [ ] **Step 1:** `bash scripts/build-host-artifacts.sh` — record exit code + stderr tail in SDD.
- [ ] **Step 2:** Scope gate: `git diff --name-only | grep -v '\.md$'` must be empty; revert non-md if any.
- [ ] **Step 3:** `bash scripts/validate.sh` and brief §14 commands; tag each failure **pre-existing** vs **introduced**.
- [ ] **Step 4:** Re-run byte baseline; compute delta vs 003 baseline and pre-edit; confirm ≤+10% or document justification in upgrade doc §10.
- [ ] **Step 5:** Write upgrade doc: per-repo traceability, adoption matrix outcome, anti-sprawl metrics, **overturn log** (#44/#45 deltas), limitations, commands run.
- [ ] **Step 6:** Score **net-stronger-at-exit** rubric (all 10 axes 1–5); pass or name limitations with user-visible impact.
- [ ] **Step 7:** Side-by-side **overturn evidence** for each ledger overturn row (before/after failing case).
- [ ] **Step 8:** Open PR `iteration/md-knowledge-v3` → `main`; Completion Report with evidence paths for QC/QA tri-review.
