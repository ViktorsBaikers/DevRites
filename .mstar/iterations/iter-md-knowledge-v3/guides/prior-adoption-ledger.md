...
| concept | source_doc | disposition | canonical_owner | action_v3 | overturn_rationale |
| --- | --- | --- | --- | --- | --- |
...
| Observable outcomes + preserving REQ/AC in Spec | 08-02 / gstack | adopted | `rite-spec/reference/spec-template.md` | strengthen | Add explicit failing case when preservation row missing evidence |
...
| Qualified backstop + cannot_verify | 08-02 / GSD | adopted | `acceptance-proof.md`, `devrites-proof-runner.md` | strengthen | Prove must name missing evidence class, not self-report |
| Explicit → active lifecycle → one implicit route | 08-02 / OMC | adopted | `intent-map.md`, `skill-authoring.md` | strengthen | Add AC-9 tie-breakers for rite-quick vs rite-build |
| Load-path placement gate (core / shared / local) | 08-02 / ECC | adopted | `skill-authoring.md` | retain | |
| Normative rewrite preservation map | 08-02 / BMAD | adopted | `skill-authoring.md` | retain | |
| Spec-quality checklist (requirements-as-tests) | 08-02 / Spec Kit | partial | `spec-checklists.md` | strengthen | Add falsifiable brownfield preservation probes |
...
| Reviewer finding fingerprints / second wave | 08-11 | rejected | Review/Seal | reject | Duplicates fresh required roles; deferred in 08-11 |
| Browser daemon / sink telemetry stack | 08-02 / benchmark | rejected | none | reject | Runtime coupling; out of Markdown scope |
| Second spec store (OpenSpec lifecycle) | 08-02 / 08-11 | rejected | none | reject | Conflicts with `.devrites/` workspace model |
| Universal TDD / worktrees / fixed review rounds | 08-02 / Superpowers | rejected | none | reject | Conflicts with evidence-scaled DevRites lifecycle |
...
| ECC catalog import (66/817 skills) | 08-11 / benchmark | rejected | none | reject | Sprawl + drift |
...
| Fixed numeric coverage / rollout thresholds | 08-11 / agent-skills | rejected | none | reject | False confidence without project baselines |
...
| Context telemetry / percentage budgets | GSD / ECC | deferred | none | defer | Engine enforcement track |
