...
- **Attention budget ≠ window size.** A large window is capacity, not attention — focus
  ...
  Aim to keep the working set for one task under ~2,000 lines. Load the files a step actually
  needs, not the ones it might; a speculative dump you never read still costs attention on
