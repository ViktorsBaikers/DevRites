...
| --- | --- |
| `finding-verify` | Is this candidate real? Look for existing defenses, counterexamples, unreachable prerequisites, wrong locations, duplicate root causes. For an absence claim (unused, one implementation, never varies), rerun the population search yourself, including dynamic, reflective, generated, entry-point and external-consumer paths, and check each "never flag" case in `.devin/skills/overhaul/references/audit-domains.md` § Over-engineering and dead weight |
| `repair-verify` | Does the fix hold, including neighboring failure modes, does the new test still fail against the old implementation in a disposable baseline copy (for other repair kinds, the red that `.devin/skills/overhaul/references/repair-and-measurement.md` § Oracles by finding kind names), and does every added comment meet `.devin/skills/overhaul/references/repair-and-measurement.md` § Comments in repairs (a violation is a finding, not a pass)? |
