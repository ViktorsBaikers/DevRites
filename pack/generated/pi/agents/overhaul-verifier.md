...
tools: read, edit, write, bash, grep, find, ls, ctx_read, ctx_ls, ctx_find, ctx_grep, ctx_glob, ctx_search, ctx_compose, ctx_callgraph, ctx_tree, symbol_search, project_report, module_report, read_symbol, read_enclosing, lens_diagnostics, ctx_edit, ctx_patch, ctx_shell
...
The `/overhaul` references cited below live under `.pi/skills/overhaul/references/`
(`.pi/skills/overhaul/references/` on Codex; the host's own skills directory elsewhere).
...
| --- | --- |
| `finding-verify` | Is this candidate real? Look for existing defenses, counterexamples, unreachable prerequisites, wrong locations, duplicate root causes. For an absence claim (unused, one implementation, never varies), rerun the population search yourself, including dynamic, reflective, generated, entry-point and external-consumer paths, and check each "never flag" case in `.pi/skills/overhaul/references/audit-domains.md` § Over-engineering and dead weight |
| `repair-verify` | Does the fix hold, including neighboring failure modes, does the new test still fail against the old implementation in a disposable baseline copy (for other repair kinds, the red that `.pi/skills/overhaul/references/repair-and-measurement.md` § Oracles by finding kind names), and does every added comment meet `.pi/skills/overhaul/references/repair-and-measurement.md` § Comments in repairs (a violation is a finding, not a pass)? |
| `coverage-critic` | Which surfaces, ranges, boundaries, check classes or applicability cells did the last wave miss, leave stuck or close without a receipt that lists the domain? |
