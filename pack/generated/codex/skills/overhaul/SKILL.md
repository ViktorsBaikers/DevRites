...
1. **Check the tools.** Require `git` and `devrites-engine` on `PATH`;
   ...
   installed (`.codex/agents/`, or the host's agent directory); if they are missing,
...
1. Dispatch `recon` ([`overhaul-recon`](.codex/agents/overhaul-recon.toml)); admit its inventory,
...
3. Launch the concurrent engineering wave: when both lanes apply,
   [`overhaul-frontend-engineer`](.codex/agents/overhaul-frontend-engineer.toml) and
   [`overhaul-backend-engineer`](.codex/agents/overhaul-backend-engineer.toml) go out in the same dispatch
   batch before you await either. Fill remaining slots with
   [`overhaul-boundary`](.codex/agents/overhaul-boundary.toml), [`overhaul-craft`](.codex/agents/overhaul-craft.toml) and
   [`overhaul-specialist`](.codex/agents/overhaul-specialist.toml) in bounded waves; reserve verifier and
...
4. Admit every receipt with `devrites-engine overhaul admit receipt`; anchor every
   ...
   ambiguous candidates to [`overhaul-verifier`](.codex/agents/overhaul-verifier.toml) (`finding-verify`)
...
2. Execute the approved DAG with [`overhaul-implementer`](.codex/agents/overhaul-implementer.toml): one
...
| Role | Agent | Mode |
| --- | --- | --- |
| Reconnaissance and coverage mapper | [`overhaul-recon`](.codex/agents/overhaul-recon.toml) | read-only |
| Frontend engineering reviewer | [`overhaul-frontend-engineer`](.codex/agents/overhaul-frontend-engineer.toml) | read-only |
| Backend engineering reviewer | [`overhaul-backend-engineer`](.codex/agents/overhaul-backend-engineer.toml) | read-only |
| Boundary and contract reviewer | [`overhaul-boundary`](.codex/agents/overhaul-boundary.toml) | read-only |
| Frontend craft, interaction and accessibility | [`overhaul-craft`](.codex/agents/overhaul-craft.toml) | read-only |
| Language, risk and other-lane specialist | [`overhaul-specialist`](.codex/agents/overhaul-specialist.toml) | read-only |
| Frontend or backend remediation implementer | [`overhaul-implementer`](.codex/agents/overhaul-implementer.toml) | writes approved paths only |
| Verifier, critic and final challenger | [`overhaul-verifier`](.codex/agents/overhaul-verifier.toml) | read-only |
