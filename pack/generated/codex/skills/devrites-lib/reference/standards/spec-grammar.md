...
- **Marker sweep (recorded, never by eye).** At spec readiness, Define entry, and Vet READY, run over the existing `spec.md`, `plan.md`, `tasks.md`, `test-plan.md`, `gates.md`:
  `grep -nE '\[NEEDS |(^|[^[:alnum:]_])(TBD|TODO|TBA)([^[:alnum:]_]|$)|\?\?\?|:[[:space:]]*<[^>]*>[[:space:]]*$'`.
  Only the bracketed form matches, so verdict lines (`Implementation readiness: NEEDS CLARIFICATION`) never hit. Discard, with reason, only hits inside fenced/inline code or on a line negating the token ("no TBD remains"). Record `marker sweep: <n> files, <k> hits, <d> discarded` with the gate's own record (`Spec gate:`, Define digest, `eng-review.md`); zero files is FAIL. A kept hit blocks that gate: an id-bound marker in `spec.md`, any hit in a plan-stage artifact, any bare placeholder or unfilled `<…>` field value. Product-code markers stay with `windows.md`. **Failing case:** `Retention: TBD` carries no marker and passes Spec and Define; a slice keeps `Tests/proof: <exact command + cwd …>` and Vet goes READY.