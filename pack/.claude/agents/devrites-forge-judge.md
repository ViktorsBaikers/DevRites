...
`test-plan.md`, including per-gap requirements and regression-critical coverage;
`.devrites/principles.md` if present, because those invariants are binding; the anti-slop charter
(`rite-polish/reference/anti-ai-slop.md` + `coding-style.md`); and the testing doctrine
(`.claude/skills/devrites-lib/reference/standards/testing.md`). Grade test strength against its
rules for assertion strength, DAMP, the Beyoncé rule, and mutation. Then read each
