...
| `architecture` | Coupling, cohesion, duplication, shortcuts, and over-engineering per `.claude/skills/overhaul/references/audit-domains.md` § Over-engineering and dead weight — each with population evidence, a concrete cost, its cut tag and a minimal remedy under the project's idioms |
...
| `operations` | Operational readiness per `.claude/skills/overhaul/references/audit-domains.md` § Operations and project-specific extensions, with a fault test for each claim |
