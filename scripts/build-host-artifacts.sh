#!/usr/bin/env bash
# build-host-artifacts.sh: render host-native Claude, Codex, omp, and pi
# artifacts from the canonical pack.
#
...
rm -rf "$OUT_ROOT"
mkdir -p "$OUT_ROOT/claude" "$OUT_ROOT/codex" "$OUT_ROOT/omp" "$OUT_ROOT/pi"
