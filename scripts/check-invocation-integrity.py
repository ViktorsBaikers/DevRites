...
RULE_REF = re.compile(
    r"(?:\.claude/skills/devrites-lib/reference/standards|\.agents/skills/devrites-lib/reference/standards|\.pi/skills/devrites-lib/reference/standards)/([a-z0-9-]+)\.md"
)
CODEX_SLASH_RITE = re.compile(
    r"(^|[^A-Za-z0-9_./-])/(rite(?:-[a-z0-9-]+)?)([^A-Za-z0-9_-]|$)"
)


def names(pattern, strip):
    return {
        os.path.basename(p)[:-strip] if strip else os.path.basename(p)
...
def scan(path, valid_names, valid_rules, problems):
    ...
    for name in set(SKILL_TOK.findall(txt)):
        if name in valid_names or name in NONSKILL or name in DOCUMENTED_NONEXISTENT:
            continue
        problems.append(
            f"{os.path.relpath(path, REPO)}: unresolved skill/agent name '{name}'"
        )
    for r in set(RULE_REF.findall(txt)):
        if r not in valid_rules:
            problems.append(
                f"{os.path.relpath(path, REPO)}: rule ref '{r}.md' not present"
            )
...
def scan_codex_sigils(path, problems):
    ...
    for lineno, line in enumerate(txt.splitlines(), 1):
        if CODEX_SLASH_RITE.search(line):
            problems.append(
                f"{os.path.relpath(path, REPO)}:{lineno}: Codex mirror uses /rite invocation; use $rite"
            )
...
def main():
    ...
    try:
        ...
        codex_rules = names(
            f"{target}/.agents/skills/devrites-lib/reference/standards/*.md", 3
        )
        codex_agents = names(f"{target}/.codex/agents/*.toml", 5)
        pi_skills = names(f"{target}/.pi/skills/*", 0)
        pi_rules = names(
            f"{target}/.pi/skills/devrites-lib/reference/standards/*.md", 3
        )
        ...
        if claude_skills != codex_skills:
            mirror.append(
                f"skills not mirrored: {sorted(claude_skills ^ codex_skills)}"
            )
        if claude_rules != codex_rules:
            mirror.append(f"rules not mirrored: {sorted(claude_rules ^ codex_rules)}")
        if claude_skills != pi_skills:
            mirror.append(
                f"skills not mirrored to pi: {sorted(claude_skills ^ pi_skills)}"
            )
        if claude_rules != pi_rules:
            mirror.append(
                f"rules not mirrored to pi: {sorted(claude_rules ^ pi_rules)}"
            )
        if claude_agents != pi_agents:
            mirror.append(
                f"agents not mirrored to pi: {sorted(claude_agents ^ pi_agents)}"
            )
        # Every public rite command must have a pi prompt stub preserving /<name>.
        public_rites = {
            s for s in claude_skills if s == "rite" or s.startswith("rite-")
        }
        if not public_rites <= pi_prompts:
            mirror.append(f"pi prompts missing: {sorted(public_rites - pi_prompts)}")

        # Claude side
        for f in glob.glob(f"{PACK}/agents/*.md") + glob.glob(
            f"{PACK}/skills/**/*.md", recursive=True
        ):
            scan(f, claude_skills | claude_agents, claude_rules, problems)
        # Codex side
        for f in glob.glob(f"{target}/.codex/agents/*.toml") + glob.glob(
            f"{target}/.agents/skills/**/SKILL.md", recursive=True
        ):
        ...
        for f in (
            glob.glob(f"{target}/.pi/agents/*.md")
            + glob.glob(f"{target}/.pi/skills/**/*.md", recursive=True)
            + glob.glob(f"{target}/.pi/prompts/*.md")
        ):
            scan(f, pi_skills | pi_agents, pi_rules, problems)

        print(
            f"invocation-integrity: {len(claude_skills)} skills / {len(claude_rules)} rules / "
        ...
        if problems or mirror:
            ...
            return 1
        print(
            "INVOCATION-INTEGRITY: PASS: every named skill/rule resolves on all harnesses."
        )
        return 0
    finally:
