...
5. Follow-ups become the next numbered local issue under `.scratch/<slug>/issues/` (or a new feature directory when scope differs); external auto-posting is out of scope. When picking the next issue to start, run `scripts/issue-ready.sh <slug>` (or unscoped for all features) instead of re-reading every file — it applies the Status/Blocked-by/claimed ready rule deterministically and prints the ready frontier.
