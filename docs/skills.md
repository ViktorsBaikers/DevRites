...
The pack ships **42 skills total** — the `rite` menu, 29 user-invocable `rite-*` workflow + utility skills, 11 model-invoked `devrites-*` specialists, plus the internal `devrites-lib` library skill (`user-invocable: false`, not a command) that ships shared references and explicit script exceptions. DevRites is distributed through the npm package (`npx devrites ...`), not through Claude or Codex plugin stores; Claude Code and Codex surfaces are generated host artifacts installed by the npm flow. The workflow control plane runs through the installed `devrites-engine` binary: orientation, read-only gates, and state mutation all use engine subcommands so Claude Code, Codex, CI, and humans get the same verdicts. Each skill is a structured workflow with its own operating rules, anti-rationalization tables, and red flags. Engineering rules live at `.claude/skills/devrites-lib/reference/standards/`; each `rite-*` skill Reads `.claude/skills/devrites-lib/reference/standards/core.md` as its first step (step 0) and pulls the other 26 rule files on demand (no carrier skill, no session-start autoload). The engine-owned installer copies generated Codex mirrors to `.agents/skills`, mirrors rules to `.agents/skills/devrites-lib/reference/standards`, installs generated `.codex/agents`, and merges Codex hooks/guidance. The npm `devrites` shim bootstraps or proxies `devrites-engine` directly (see [`cli.md`](cli.md)).

**Naming convention.** `rite-*` is the user-facing slash-command surface (lifecycle phases plus utilities — `rite-quick`, `rite-frame`, `rite-adopt`, `rite-learn`, `rite-customize`, `rite-explain`, `rite-pov`, `rite-dogfood`, `rite-pr-feedback`, `rite-doctor`, `rite-prototype`, `rite-handoff`, `rite-zoom-out`, `rite-pressure-test`). Some niche utilities are explicit-only (`disable-model-invocation: true`) to keep the always-loaded skill surface small; invoke them with `/rite <verb>` / `/rite-<verb>` or Codex `$rite <verb>` / `$rite-<verb>`. `devrites-*` is internal (model-invoked, hidden from the menu) and collision-avoiding against bundled Claude Code skill names. Visibility is governed by each skill's `user-invocable:` flag; the prefix mirrors it.
...
phase (the rationalizations in `pack/.claude/skills/devrites-lib/reference/standards/anti-patterns.md`, specialized), not a
...
| `rite-customize` | `/rite customize` | `/rite-customize` | `$rite customize` | `$rite-customize` |
| `rite-explain` | `/rite explain` | `/rite-explain` | `$rite explain` | `$rite-explain` |
| `rite-pov` | `/rite pov` | `/rite-pov` | `$rite pov` | `$rite-pov` |
| `rite-dogfood` | `/rite dogfood` | `/rite-dogfood` | `$rite dogfood` | `$rite-dogfood` |
| `rite-pr-feedback` | `/rite pr-feedback` | `/rite-pr-feedback` | `$rite pr-feedback` | `$rite-pr-feedback` |
...
The progress command owns the deterministic header, slice meter, and flow ribbon; the
chat reply only summarizes status: `Done`, `Changed`, `Evidence`, `Open`, one `Next`
command, `Record`, and `↻ Hygiene`. Detailed reports stay in durable workspace
artifacts such as `spec.md`, `plan.md`, `evidence.md`, `review.md`, `seal.md`, and
`ship.md`.
...
| `/rite spec <feature>` | `/rite-spec <feature>` | spec | **Start here.** Deep investigation → writes `spec.md`: understands the ask fully, decides **placement** (where it lives), names what it resolves, closes gaps with you (questions with options), and gathers any design references you optionally attach (screenshots / Figma / links / video — there may be none). Checks the shipped archive for prior art first, so a solved feature is extended rather than re-specced. |
...
| `/rite autocomplete` | `/rite-autocomplete` | (orchestrator) | Runs the whole lifecycle unattended (spec → … → seal → ship), choosing the best option at each soft gate. Arms AFK + checkpoint mode for the run. `--ship` / `--yolo` auto-confirms the final type-`GO`. |
...
| `/rite customize` | `/rite-customize` | utility | Guided authoring for `.devrites/overrides` and `.devrites/extensions`; writes the smallest project-local customization and validates it. |
| `/rite explain` | `/rite-explain` | utility | Human learning loop — turn a concept, diff, idea, or recent work into a dense explainer or a concern-ordered change walkthrough plus optional active-recall check-in. |
| `/rite pov` | `/rite-pov` | utility | Project-grounded verdict on adopting, switching, rejecting, or ignoring a named external technology/library/platform/pattern. |
| `/rite dogfood` | `/rite-dogfood` | utility | Diff-scoped browser QA by changed user journey, with safe small-fix loop and durable report. |
| `/rite pr-feedback` | `/rite-pr-feedback` | utility | Fetch, judge, fix, reply to, and resolve GitHub PR review threads. |
| `/rite doctor` | `/rite-doctor` | diagnostic | Diagnose DevRites install + workspace health — install integrity, a stale `.devrites/ACTIVE`, a corrupt workspace, orphaned gates, broken hook wiring, and in-progress merge/rebase state. |

The 11 model-invoked internal specialists (hidden from the menu): `devrites-interview`, `-source-driven`, `-doubt`, `-ux-shape`, `-frontend-craft`, `-prose-craft`, `-browser-proof`, `-debug-recovery`, `-api-interface`, `-audit` (security / perf / simplify), `-refresh-indexes`. Plus the `devrites-lib` library skill (scripts, not a command). The `/rite` menu carries the routing, the user-invocable utilities all use the `rite-*` prefix (`rite-quick`, `rite-frame`, `rite-adopt`, `rite-learn`, `rite-customize`, `rite-explain`, `rite-pov`, `rite-dogfood`, `rite-pr-feedback`, `rite-doctor`, `rite-prototype`, `rite-handoff`, `rite-zoom-out`, `rite-pressure-test`, `rite-autocomplete`), and the `/rite-polish` phases live as references inside the same skill rather than as `rite-polish-code` / `rite-polish-ui`. Triggers and interactions are documented in [`command-map.md`](command-map.md); diagrams in [`flow.md`](flow.md).
...
| [`rite-doctor`](../pack/.claude/skills/rite-doctor/SKILL.md) | Diagnose DevRites install + workspace health — install integrity, stale `.devrites/ACTIVE`, corrupt workspace, orphaned gates, broken hook wiring, and in-progress merge/rebase state. Read-only report. | "rite doctor", "is DevRites healthy", "why isn't the workflow picking up my feature". |
...
| [`rite-zoom-out`](../pack/.claude/skills/rite-zoom-out/SKILL.md) | Map an unfamiliar area — modules, callers, callees, decisions — in the project's domain glossary. | Explicit-only: `/rite-zoom-out` / `/rite zoom-out`. |
| [`rite-prototype`](../pack/.claude/skills/rite-prototype/SKILL.md) | Throwaway code answering ONE design question — logic harness OR 2–4 UI variations on one route. | Explicit-only: `/rite-prototype` / `/rite prototype`. |
...
| Skill | What It Does | Use When |
|---|---|---|
| (engineering rules) | Live at `.claude/skills/devrites-lib/reference/standards/` post-install — each `rite-*` skill Reads `.claude/skills/devrites-lib/reference/standards/core.md` as its first step (step 0); the other 26 rule files (`coding-style.md`, `prose-style.md`, `error-handling.md`, `testing.md`, `spec-grammar.md`, `code-review.md`, `edge-case-trace.md`, `principles.md`, `security.md`, `performance.md`, `observability.md`, `developer-experience.md`, `patterns.md`, `git-workflow.md`, `ci-cd.md`, `hooks.md`, `documentation.md`, `development-workflow.md`, `deprecation.md`, `elicitation.md`, `agents.md`, `context-hygiene.md`, `anti-patterns.md`, `afk-hitl.md`, `tooling.md`, `skill-authoring.md`) load on demand per skill body. No carrier skill, no session-start autoload. | n/a — step-0 core / on-demand by path. |
