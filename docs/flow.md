...
erDiagram
    ...
    state {
        ...
        string sequence_parent "deferred-ship chain: preceding workspace slug"
        int sequence_position "1-based ordinal in the recorded continuation chain"
        int sequence_workspaces_remaining "chain slots still open after this workspace; never negative"
        string sequence_role "release on the release milestone; gates then require the chain union"
