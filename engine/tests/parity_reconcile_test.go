...
// captures the worktree as a tree object, check diffs it and flags source changes
// outside the claimed manifest. Runs git in a real repo, so stdout + exit are
// captured from the live command. The .devrites/ bookkeeping is skipped by the
// gate, so it never affects the verdict.
func TestParityReconcile(t *testing.T) {
	...
	snapshot := func(t *testing.T, work string) {
	...
	t.Run("check-clean", func(t *testing.T) {
		...
		writeFeatureFile(t, work, "feat", ".reconcile-claimed", "src/a.js\n")
		check(t, work)
	})
	...
	t.Run("check-no-base", func(t *testing.T) {
		work := newRepo(t)
		writeFeatureFile(t, work, "feat", ".reconcile-claimed", "src/a.js\n")
		check(t, work) // no snapshot -> exit 6
	})

	t.Run("check-no-claimed", func(t *testing.T) {
		work := newRepo(t)
		snapshot(t, work)
		check(t, work) // base present, no claimed -> exit 6
	})

	t.Run("inline-fallback", func(t *testing.T) {
		work := newRepo(t)
		snapshot(t, work)
		writeFeatureFile(t, work, "feat", ".reconcile-inline", "")
		check(t, work) // gate skipped -> exit 0
	})
